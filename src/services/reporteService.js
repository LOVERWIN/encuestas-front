...
export const getReporte = (slug) => {
  return apiClient.get(`/encuestas/${slug}/reporte`);
};

export const getRespuestasAbiertas = (preguntaId, page = 1) => {
  return apiClient.get(`/preguntas/${preguntaId}/respuestas?page=${page}`);
};

export const exportReporte = (slug, format) => {
  return apiClient.get(`/encuestas/${slug}/export?format=${format}`, {
